...
class XNAT_configure:
    ...
    def configure_project(self, project_path, project_url, username, password):
        ...
        for project in root.findall('xnat:projectData', namespaces):
            project_id = project.find('xnat:ID', namespaces).text
            ...
            print(f"Status project {project_id} create:", response.status_code)
            put_response = requests.put(f"{project_url}/{project_id}", data=project_data, headers=self.project_headers, auth=HTTPBasicAuth(username, password))
            print(f"Status project {project_id} initialize:", put_response.status_code)
